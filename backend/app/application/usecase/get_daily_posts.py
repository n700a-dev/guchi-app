...
from application.query_service.posts_query_service import PostsQueryService
...
class GetDailyPosts(BaseUseCase):
    """日ごとのグチを取得する。"""
    ...
    def execute(self, postedDate: str) -> List[PostObject]:
        posts = (
            session.query(PostRepository)
            .filter(PostRepository.author_id == self.author_id)
            .filter(PostRepository.posted_date == postedDate)
            .order_by(PostRepository.created_at_ms.desc())
            .all()
        )
        k = [
            PostObject(
